    ...
    return (
        <form onSubmit={handleSubmit} className="bg-gray-50 p-4 rounded mt-4">
            ...
            <div className="flex flex-col gap-2">
                <select value={fileType} onChange={(e) => setFileType(e.target.value as FileType)} className="px-3 py-2 border rounded">
                    <option value="SOP">Statement of Purpose (SOP)</option>
                    <option value="Transcript">Transcript</option>
                    <option value="Passport">Passport Copy</option>
                </select>
                <input type="file" accept=".pdf" onChange={(e) => setFile(e.target.files?.[0] || null)} className="border rounded p-1" required />
